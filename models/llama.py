...
def _prepare_decoder_attention_mask_factory(
        original_fn, efficient_implementation, local_branch, global_branch):

    def _product_fn(attention_mask, input_shape,
                    inputs_embeds, past_key_values_length):
        if efficient_implementation:
            return None
        combined_attention_mask = original_fn(
            attention_mask, input_shape, inputs_embeds, past_key_values_length)
        dtype = combined_attention_mask.dtype

        _, _, q_len, kv_seq_len = combined_attention_mask.shape
        window = torch.tril(torch.ones_like(combined_attention_mask) *
                            torch.finfo(dtype).min,
                            -local_branch+kv_seq_len-q_len)
        window[..., :global_branch] = 0
        combined_attention_mask = combined_attention_mask + window
...
def attn_forward_factory_efficient(
    ...
    def limited_distance_forward(
        ...
        # original codes:
        # rot_query_states, rot_key_states = apply_rotary_pos_emb(
            ...
            for head_i in range(self.num_heads):
                ...
                if attention_mask is not None:
                    ...
                    attn_weights = torch.max(
                        attn_weights, torch.tensor(torch.finfo(attn_weights.dtype).min, device=device)
                    )
                query_states[:, head_i] = torch.matmul(
                    F.softmax(attn_weights, dim=-1).to(dtype),
                    value_states[:, head_i]
                )
...
def attn_forward_factory(self, use_lambda_mask, local_branch, global_branch,
                         limit_distance, triangle_offset):
    ...
    def limited_distance_forward(
        ...
        for head_i in range(self.num_heads):
            ...
            if limit_distance is not None:
                ...
                # a band of attention at the first few rows
                if use_lambda_mask:
                    attn_weights[:, :, :global_branch] += \
                        stationary_query_states[:, head_i].matmul(
                            stationary_key_states[:, head_i, :global_branch].transpose(1, 2)
            ...
            if attn_weights.size() != (bsz, q_len, kv_seq_len):
                raise ValueError(
                    f"Attention weights should be of size {(bsz, q_len, kv_seq_len)}, but is"
                    f" {attn_weights.size()}"
                )
            ...
            # original codes: upcast attention to fp32
...
class LLAMA_Model(Model_Base):
    ...
    def __init__(
        ...
        use_lambda_mask, efficient_implementation, local_branch, global_branch,
        limit_distance, triangle_offset
    ):
        ...
        if use_lambda_mask:
            self.model.model._prepare_decoder_attention_mask = \
                _prepare_decoder_attention_mask_factory(
                    self.model.model._prepare_decoder_attention_mask,
                    efficient_implementation, local_branch, global_branch,
                )

        for hidden_layer in self.model.model.layers:
            attn = hidden_layer.self_attn
            factory_class = attn_forward_factory_efficient \
                if efficient_implementation \
                else attn_forward_factory
            attn.forward = factory_class(
