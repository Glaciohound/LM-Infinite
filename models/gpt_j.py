...
def attn_forward_factory(
    ...
    def _attn(
        ...
        # query_rot = query_rot.to(torch.float32)
        # key_rot = key_rot.to(torch.float32)
        # query_stationary = query_stationary.to(torch.float32)
        # key_stationary = key_stationary.to(torch.float32)
        ...
        for head_i in range(self.num_attention_heads):
            ...
            value[:, head_i] = torch.matmul(attn_weights, value[:, head_i])
    ...
    def __init__(
        ...
        for hidden_layer in self.model.transformer.h:
            ...
            if use_lambda_mask:
                ...
                triangle[:, :global_branch] = 0
                attention_mask = attention_mask.logical_xor(triangle)
            attention_mask = attention_mask.view(1, 1, max_length, max_length)
